...
def export_jsonl(base_url: str, token: str, task_id: str, output_root: Path) -> list[Path]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export archived task trace JSONL files")
    ...
    exported = export_jsonl(base_url, token, args.task_id, args.output)
    if not exported:
        raise SystemExit("trace archive contains no JSONL files")
    for path in exported:
        print(path)
    print(f"Exported {len(exported)} JSONL file(s) to {args.output}")
