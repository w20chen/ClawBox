"""Bridge eBPF exec-image telemetry to static mvdan-clause observations.

The eBPF collector's ``(host_pid, exec_seq)`` object is a runtime *exec-image
...
# Kept in sync with the eBPF collector's windowing constants.
...
class ExecImageRecord:
    """One runtime exec-image occurrence produced by the eBPF collector.
