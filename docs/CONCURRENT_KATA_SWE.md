# Concurrent Kubernetes + Kata SWE-Rebench compatibility path
...
Secret, boots a smoke microVM with the selected RuntimeClass, and runs one task. When `kata-fc`
is explicitly selected on Minikube it also creates the persistent containerd devmapper thin pool and installs a systemd unit that
...
## Decision: reuse current ClawTune v2 at a versioned asset boundary
...
`swe_rebench/.runtime/assets`: OpenClaw integration assets, plugin, sidecar, setup and task entrypoint. ClawBox packages
...
The nodes must have containerd, KVM, Kata Containers, and the selected RuntimeClass handler.
...
When Firecracker is selected, it requires a block-device rootfs. The Kata Deploy chart maps `kata-fc` to the
...
Agent Pods instead of Docker containers. It creates one namespace per tenant, uses the configured RuntimeClass,
...
Unit tests validate the Pod/Job contract, secret references, RuntimeClass, asset injection and
Guaranteed QoS shape. A real end-to-end pass requires an arm64 KVM host and a functioning
Kata RuntimeClass; Windows development machines cannot validate a microVM boot.
