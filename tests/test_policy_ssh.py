...
def test_policy_rejects_duplicate_admission_before_spawning_ssh(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Idempotent admission must not duplicate the Tool side effect."""
    monkeypatch.setenv("CLAWBOX_REAL_SSH", "/fake/ssh")
    monkeypatch.setenv("CLAWBOX_POLICY_CONTROL_URL", "http://policy.test")
    monkeypatch.setenv("CLAWBOX_POLICY_CONTROL_TOKEN", "token")
    monkeypatch.setenv("CLAWBOX_POLICY_SESSION_ID", "session-a")
    ...
    def post(path: str, _body: dict, *, attempts: int) -> dict:
        posted.append(path)
        return {
            "decision": "ADMIT", "duplicate": True, "sandbox_id": "tool-a",
            "epoch": 1, "container_port": 2222,
            "host": "192.0.2.20", "port": 20020,
        }

    def fail_spawn(_argv: list[str]):
        raise AssertionError("duplicate admission reached subprocess")
