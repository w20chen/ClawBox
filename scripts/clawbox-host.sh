...
PLATFORM_IMAGE_ENV="${CLAWBOX_PLATFORM_IMAGE_ENV:-${ROOT}/.artifacts/platform-images.env}"
DEFAULT_DATABASE_URL="sqlite:////data/clawbox.db"
DEPLOYMENTS=(
  clawbox-ingester
  clawbox-tune-kb
...
CONFIG_TOOL_IMAGE="${CLAWBOX_TOOL_IMAGE:-${TOOL_IMAGE:-}}"
CONFIG_LLM_EGRESS_CIDR="${CLAWBOX_LLM_EGRESS_CIDR:-}"
CONFIG_PROFILE="${CLAWBOX_PROFILE:-small}"
CONFIG_CONTROL_IMAGE="${CLAWBOX_CONTROL_IMAGE:-${CONTROL_IMAGE:-}}"
CONFIG_RUNTIME_IMAGE="${CLAWBOX_RUNTIME_IMAGE:-${RUNTIME_IMAGE:-}}"
CONFIG_TOOL_BRIDGE_IMAGE="${CLAWBOX_TOOL_BRIDGE_IMAGE:-${TOOL_BRIDGE_IMAGE:-}}"
...
configure options:
  --database-url URL             PostgreSQL psycopg URL (default: local SQLite)
...
Platform images are also read from CONTROL_IMAGE, RUNTIME_IMAGE and
TOOL_BRIDGE_IMAGE, or from .artifacts/platform-images.env generated by the
platform image build. TOOL_IMAGE is accepted as an alias for CLAWBOX_TOOL_IMAGE.
EOF
}
...
doctor() {
  require kubectl
  require "${PYTHON}"
  local failed=0 arch node_ready label registry deployment ready desired image images_ok=1
  arch="$(uname -m)"
  ...
  registry="SKIP (external registry supported)"
  ...
  for deployment in "${DEPLOYMENTS[@]}"; do
    image="$(deployment_container_image "${deployment}")"
    immutable_image "${image}" || images_ok=0
    while IFS= read -r image; do
      [[ -z "${image}" ]] || immutable_image "${image}" || images_ok=0
    done < <(deployment_init_images "${deployment}")
  done
  for image in "$(controller_env_image RUNTIME_IMAGE)" "$(controller_env_image TOOL_BRIDGE_IMAGE)"; do
    immutable_image "${image}" || images_ok=0
  done
  if (( images_ok )); then
    printf '%-22s %s\n' platform-images PASS
  else
    printf '%-22s %s\n' platform-images "FAIL (mutable or missing)"
    failed=1
  fi
...
platform_image_from_file() {
  local key="$1"
  [[ -f "${PLATFORM_IMAGE_ENV}" ]] || return 0
  sed -n "s/^${key}=//p" "${PLATFORM_IMAGE_ENV}" | tail -n 1
}

ensure_rendered_manifests() {
  local control runtime tool_bridge
  printf 'Rendering immutable deployment manifests...\n'
  CONFIG_CONTROL_IMAGE="${CONFIG_CONTROL_IMAGE:-$(platform_image_from_file CONTROL_IMAGE)}"
  CONFIG_RUNTIME_IMAGE="${CONFIG_RUNTIME_IMAGE:-$(platform_image_from_file RUNTIME_IMAGE)}"
  CONFIG_TOOL_BRIDGE_IMAGE="${CONFIG_TOOL_BRIDGE_IMAGE:-$(platform_image_from_file TOOL_BRIDGE_IMAGE)}"
  ...
  "${PYTHON}" "${ROOT}/scripts/render-kubernetes-images.py" \
...
migrate_legacy_managed_secret() {
  ...
  legacy="$(kubectl -n "${TASK_NAMESPACE}" get sandboxtasks -o json | "${PYTHON}" -c '
...
')"
  ...
  "${PYTHON}" - "${tool_image}" "${llm_secret}" "${runtime_image}" "${llm_cidr}" "${profile}" \
...
parse_configure_args() {
  while (( $# )); do
    case "$1" in
      --database-url) require_option_value "$@"; CONFIG_DATABASE_URL="$2"; shift 2 ;;
      --llm-api-key) require_option_value "$@"; CONFIG_LLM_API_KEY="$2"; shift 2 ;;
      --llm-base-url) require_option_value "$@"; CONFIG_LLM_BASE_URL="$2"; shift 2 ;;
      --llm-model) require_option_value "$@"; CONFIG_LLM_MODEL="$2"; shift 2 ;;
      --openclaw-model-ref) require_option_value "$@"; CONFIG_OPENCLAW_MODEL_REF="$2"; shift 2 ;;
      --tool-image) require_option_value "$@"; CONFIG_TOOL_IMAGE="$2"; shift 2 ;;
      --llm-egress-cidr) require_option_value "$@"; CONFIG_LLM_EGRESS_CIDR="$2"; shift 2 ;;
      --profile) require_option_value "$@"; CONFIG_PROFILE="$2"; shift 2 ;;
      --control-image) require_option_value "$@"; CONFIG_CONTROL_IMAGE="$2"; shift 2 ;;
      --runtime-image) require_option_value "$@"; CONFIG_RUNTIME_IMAGE="$2"; shift 2 ;;
      --tool-bridge-image) require_option_value "$@"; CONFIG_TOOL_BRIDGE_IMAGE="$2"; shift 2 ;;
      -h|--help) usage; exit 0 ;;
...
validate_configure_inputs() {
  local value
  for value in \
    "${CONFIG_DATABASE_URL}" "${CONFIG_LLM_API_KEY}" "${CONFIG_LLM_BASE_URL}" \
    "${CONFIG_LLM_MODEL}" "${CONFIG_OPENCLAW_MODEL_REF}" "${CONFIG_TOOL_IMAGE}" \
    "${CONFIG_LLM_EGRESS_CIDR}" "${CONFIG_CONTROL_IMAGE}" \
    "${CONFIG_RUNTIME_IMAGE}" "${CONFIG_TOOL_BRIDGE_IMAGE}"; do
    [[ "${value}" != *$'\n'* && "${value}" != *$'\r'* ]] \
      || die "configuration values must not contain newlines"
  done
  if [[ -n "${CONFIG_DATABASE_URL}" ]]; then
    [[ "${CONFIG_DATABASE_URL}" == sqlite:* \
      || "${CONFIG_DATABASE_URL}" == postgresql+psycopg://* ]] \
      || die "--database-url must use sqlite: or postgresql+psycopg://"
  fi
  ...
  if [[ -n "${CONFIG_LLM_BASE_URL}" ]]; then
    "${PYTHON}" - "${CONFIG_LLM_BASE_URL}" <<'PY' >/dev/null
import sys
from urllib.parse import urlsplit
try:
    value = urlsplit(sys.argv[1])
except ValueError as exc:
    raise SystemExit(f"LLM base URL is invalid: {exc}") from exc
if value.scheme not in {"http", "https"} or not value.netloc:
    raise SystemExit("LLM base URL must be an absolute http(s) URL")
PY
  fi
  if [[ -n "${CONFIG_TOOL_IMAGE}" ]]; then
    immutable_image "${CONFIG_TOOL_IMAGE}" \
      || die "--tool-image must use IMAGE@sha256:<64 lowercase hex>"
    [[ -n "${CONFIG_LLM_EGRESS_CIDR}" ]] \
      || die "--llm-egress-cidr is required with --tool-image"
  fi
  if [[ -n "${CONFIG_LLM_EGRESS_CIDR}" ]]; then
    "${PYTHON}" - "${CONFIG_LLM_EGRESS_CIDR}" <<'PY' >/dev/null
import ipaddress, sys
try:
    network = ipaddress.ip_network(sys.argv[1], strict=True)
except ValueError as exc:
    raise SystemExit(f"LLM egress CIDR is invalid: {exc}") from exc
if network.prefixlen == 0:
    raise SystemExit("LLM egress CIDR must not allow the entire internet")
PY
  fi
}
...
ensure_base_secrets() {
  local value current service_token ingest_secret
  if ! kubectl -n "${SYSTEM_NAMESPACE}" get secret clawbox-control-plane >/dev/null 2>&1; then
    if [[ -z "${CONFIG_DATABASE_URL}" ]]; then
      CONFIG_DATABASE_URL="${DEFAULT_DATABASE_URL}"
      printf 'Using persistent single-node SQLite; set --database-url for PostgreSQL.\n'
    fi
    require openssl
    start_setup_dir
  ...
  elif [[ -n "${CONFIG_DATABASE_URL}" ]]; then
    current="$(secret_value "${SYSTEM_NAMESPACE}" clawbox-control-plane database-url)"
    if [[ "${current}" != "${CONFIG_DATABASE_URL}" ]]; then
      service_token="$(secret_value "${SYSTEM_NAMESPACE}" clawbox-control-plane service-token)"
      ingest_secret="$(secret_value "${SYSTEM_NAMESPACE}" clawbox-control-plane ingest-secret)"
      start_setup_dir
      printf '%s' "${CONFIG_DATABASE_URL}" >"${SETUP_TEMP_DIR}/database-url"
      printf '%s' "${service_token}" >"${SETUP_TEMP_DIR}/service-token"
      printf '%s' "${ingest_secret}" >"${SETUP_TEMP_DIR}/ingest-secret"
      kubectl -n "${SYSTEM_NAMESPACE}" create secret generic clawbox-control-plane \
        --from-file="database-url=${SETUP_TEMP_DIR}/database-url" \
        --from-file="service-token=${SETUP_TEMP_DIR}/service-token" \
        --from-file="ingest-secret=${SETUP_TEMP_DIR}/ingest-secret" \
      ...
      deployment_exists clawbox-ingester \
        && kubectl -n "${SYSTEM_NAMESPACE}" rollout restart deployment/clawbox-ingester >/dev/null
      printf 'Updated the control-plane database URL and preserved generated tokens.\n'
    fi
  fi
  ...
  if ! kubectl -n "${TASK_NAMESPACE}" get secret clawbox-llm >/dev/null 2>&1; then
  ...
  elif [[ -n "${CONFIG_LLM_API_KEY}${CONFIG_LLM_BASE_URL}${CONFIG_LLM_MODEL}${CONFIG_OPENCLAW_MODEL_REF}" ]]; then
    CONFIG_LLM_API_KEY="${CONFIG_LLM_API_KEY:-$(secret_value "${TASK_NAMESPACE}" clawbox-llm llm-api-key)}"
    CONFIG_LLM_BASE_URL="${CONFIG_LLM_BASE_URL:-$(secret_value "${TASK_NAMESPACE}" clawbox-llm llm-upstream-base-url)}"
    CONFIG_LLM_MODEL="${CONFIG_LLM_MODEL:-$(secret_value "${TASK_NAMESPACE}" clawbox-llm llm-model)}"
    CONFIG_OPENCLAW_MODEL_REF="${CONFIG_OPENCLAW_MODEL_REF:-$(secret_value "${TASK_NAMESPACE}" clawbox-llm openclaw-model-ref)}"
    for value in CONFIG_LLM_API_KEY CONFIG_LLM_BASE_URL CONFIG_LLM_MODEL CONFIG_OPENCLAW_MODEL_REF; do
      [[ -n "${!value}" ]] || die "existing clawbox-llm is missing ${value}"
    done
    start_setup_dir
    printf '%s' "${CONFIG_LLM_API_KEY}" >"${SETUP_TEMP_DIR}/llm-api-key"
    printf '%s' "${CONFIG_LLM_BASE_URL}" >"${SETUP_TEMP_DIR}/llm-upstream-base-url"
    ...
    kubectl apply -f "${SETUP_TEMP_DIR}/secret.yaml" >/dev/null
    cleanup_setup_files; trap - EXIT
    printf 'Updated Secret %s/clawbox-llm; new tasks use the rotated values.\n' "${TASK_NAMESPACE}"
  fi
}
...
PY
  ...
  "${PYTHON}" - "${CONFIG_TOOL_IMAGE}" clawbox-llm "${runtime_image}" \
...
normalize_managed_database_secret() {
  ...
  if [[ "${current}" != "${desired}" && "${current}" != sqlite:* \
      && -z "${CONFIG_DATABASE_URL}" ]]; then
    return
  fi
  start_setup_dir
...
configure() {
  parse_configure_args "$@"
  require kubectl; require "${PYTHON}"; require base64
  wait_for_cluster
  validate_configure_inputs
  ensure_rendered_manifests
  ensure_namespaces
...
ensure_capacity_configmap() {
  ...
  sudo env KUBECONFIG="${kubeconfig}" \
    PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    "${PYTHON}" "${ROOT}/scripts/collect-node-capacity.py" --configmap \
    >"${SETUP_TEMP_DIR}/capacity.yaml"
...
up() {
  ...
  if all_deployments_exist; then
    printf 'Existing ClawBox deployment found; reconciling it in place...\n'
  else
    printf 'Preparing the first ClawBox deployment...\n'
  fi
  preflight_reconcile
  run_migrations
...
local_api() {
  ...
  local token ready=0 local_port=""
  token="$(secret_value "${SYSTEM_NAMESPACE}" clawbox-managed service-token)"
  ...
  kubectl -n "${SYSTEM_NAMESPACE}" port-forward \
    service/clawbox-managed-api :8085 >"${LOCAL_API_LOG_FILE}" 2>&1 &
  LOCAL_API_PORT_FORWARD_PID=$!
  trap cleanup_local_api EXIT
  for _ in $(seq 1 30); do
    local_port="$(sed -nE 's/^Forwarding from .*:([0-9]+) -> 8085$/\1/p' \
      "${LOCAL_API_LOG_FILE}" | head -n 1)"
    if [[ "${local_port}" =~ ^[0-9]+$ ]] \
  ...
  export CLAWBOX_API_URL="http://127.0.0.1:${local_port}"
